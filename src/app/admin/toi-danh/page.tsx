...
export default function ToiDanhPage() {
  ...
  useEffect(() => {
    document.title = "Danh mục Tội danh | QLCNC";
  }, []);
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await toimDanhApi.getAll(params);
      ...
      console.error("Lỗi khi tải danh sách tội danh:", error);
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: "Không thể tải danh sách tội danh",
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (id: string, ten: string) => {
    const result = await Swal.fire({
      title: "Xác nhận xóa?",
      text: `Bạn có chắc chắn muốn xóa tội danh "${ten}"?`,
      icon: "warning",
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire({
          icon: "error",
          title: "Lỗi!",
          text: error.message || "Có lỗi xảy ra khi xóa tội danh",
        });
      }
    }
  };
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Quản lý danh mục các tội danh vi phạm pháp luật
          </p>
        </div>
        {hasPermission("toi-danh:create") && (
          <Link
            href="/admin/toi-danh/them-moi"
          ...
          <>
                  ...
                  {displayData.map((item) => (
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                        <div className="flex gap-2">
                          {hasPermission("toi-danh:update") && (
                            <IconButton
                              icon={<PencilIcon />}
                              tooltip="Chỉnh sửa"
                              variant="edit"
                              href={`/admin/toi-danh/${item.id}/chinh-sua`}
                            />
                          )}
                          {hasPermission("toi-danh:delete") && (
                            <IconButton
                              icon={<TrashBinIcon />}
                              tooltip="Xóa"
                              variant="delete"
            ...
            <div className="px-6 py-4 border-t border-gray-200 dark:border-gray-700 flex items-center justify-between">
              <div className="text-sm text-gray-700 dark:text-gray-300">
                Hiển thị {startIndex} - {endIndex} trong tổng {total} tội danh
              </div>
              <div className="flex gap-2">
                <button
