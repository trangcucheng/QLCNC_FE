...
export default function ChinhSuaBieuMauPage() {
  ...
  useEffect(() => {
    document.title = "Chỉnh sửa Biểu mẫu | QLCNC";
    if (id) {
      fetchBieuMau();
    }
  ...
  const fetchBieuMau = async () => {
    try {
      ...
      const response = await bieuMauApi.getById(id);
      
      if (response) {
        setFormData({
          ten: response.ten || "",
          moTa: response.moTa || "",
          duongDan: response.duongDan || "",
          phienBan: response.phienBan || "",
        });
      }
    } catch (error: any) {
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: error.message || "Không thể tải thông tin biểu mẫu",
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      await bieuMauApi.update(id, formData);
      ...
      Swal.fire({
        icon: "error",
        title: "Lỗi!",
        text: error.message || "Có lỗi xảy ra khi cập nhật biểu mẫu",
      });
    } finally {
      setLoading(false);
    }
